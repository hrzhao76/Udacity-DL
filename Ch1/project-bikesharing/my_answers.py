...
class NeuralNetwork(object):
    def __init__(self, input_nodes, hidden_nodes, output_nodes, learning_rate):
        ...
        #
        def sigmoid(x):
            ### TODO: WHY am I having error here?
            ### Input Features.
    ...
    def train(self, features, targets):
        ...
        delta_weights_i_h = np.zeros(
            self.weights_input_to_hidden.shape, dtype=np.float64
        )
        delta_weights_h_o = np.zeros(
            self.weights_hidden_to_output.shape, dtype=np.float64
        )
        for X, y in zip(features, targets):
            ### NOTE: This zip makes X the shape of (n_features,) and y the shape of (n_records,)
            ### This is very important for the shape!!!!
            ### X.dtype == object , contains True, False
            ### y.dtype == object
            X = np.array(X, dtype=np.float64)
    ...
    def backpropagation(
        ...
        delta_weights_i_h += (X[:, None] @ hidden_error_term.T).astype(
            np.float64
        )
